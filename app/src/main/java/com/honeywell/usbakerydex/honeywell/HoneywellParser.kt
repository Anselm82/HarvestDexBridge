package com.honeywell.usbakerydex.honeywell

import android.util.Log
import com.honeywell.usbakerydex.DexConnectionService
import com.honeywell.usbakerydex.dex.model.*
...
class HoneywellParser {
    ...
    companion object {
        ...
        fun readG83(jsonItem: JSONObject, sequenceNumber: Int? = 1, ucsType: String): G83? {
            val id = extract<String>(
                jsonItem,
                HKey._06,
                null
            )
            val upc = extract<String>(
                jsonItem,
                HKey._04,
                null
            )
            ...
            return G83(sequenceNumber,
                ...
                extract(
                    jsonItem,
                    HKey._08,
                    if (ucsType == BASE_RECORD) 0.0 else -1.0
                ),
                extract(
                    jsonItem,
                    HKey._09,
        ...
        fun readConfiguration(rootJsonObject: JSONObject): Configuration? {
            val config = getIgnoreCase(
                rootJsonObject,
                HKey.CONFIG
            )
            if (config != null && rootJsonObject.has(config)) {
                val configJsonObject = rootJsonObject.getJSONObject(config)
                val supplier = getIgnoreCase(
                    configJsonObject,
                    HKey.SUPPLIER
                )
                val retailer = getIgnoreCase(
                    configJsonObject,
                    HKey.RETAILER
                )
        ...
        fun readTransaction(rootJsonObject: JSONObject): DEXTransaction? {
            return DEXTransaction.readTransaction(
                rootJsonObject
            )
        }
    }
